...
fn csr(pki: &PrivateKeyInfo<'_>, exts: Vec<Extension>) -> anyhow::Result<Vec<u8>> {
    ...
    let signature = BitString::from_bytes(sig.as_ref()).context("failed to parse signature")?;
    CertReq {
        info,
        algorithm,
        signature,
...
pub fn selfsigned(key: impl AsRef<[u8]>) -> anyhow::Result<Vec<Vec<u8>>> {
    ...
    let rdns = RdnSequence::from_str("CN=localhost")?.to_der()?;
    ...
    let sig = pki.sign(&tbs.to_der()?, &alg)?;
    let crt = Certificate {
        tbs_certificate: tbs,
