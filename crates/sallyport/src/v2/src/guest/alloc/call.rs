...
use super::{Allocator, Collect, Collector, Commit, Committer, InRef, Stage};
...
pub trait Call<'a> {
    ...
    fn collect(committed: Self::Committed, col: &impl Collector) -> Self::Collected;
}
...
impl<'a, T: Call<'a>> Stage<'a> for T {
    type Item = StagedCall<'a, T>;
