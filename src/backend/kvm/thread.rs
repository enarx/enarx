...
use super::KVM_HC_MAP_GPA_RANGE;
#[cfg(feature = "gdb")]
...
fn execute_page_state_change<P>(
    gpa: u64,
    num_pages: u64,
    keep: &mut Keep<P>,
    private: bool,
    ...
    P: KeepPersonality,
{
    ...
    let region = keep
        ...
        .context("Couldn't find memory region")?;
    ...
    let res = unsafe {
        fallocate(
            ...
            mode,
            offset as i64,
            num_pages as i64 * 0x1000,
        )
    };
    ensure!(res == 0, "fallocate call failed");

    // Switch the memory.
    set_memory_attributes(&mut keep.vm_fd, gpa.as_u64(), num_pages * 0x1000, private)
        .context("Failed to update memory attributes")?;
    ...
    if private {
        ...
        let res = unsafe { madvise(addr, num_pages as usize * 0x1000, MADV_FREE) };
        ensure!(res == 0, "Failed to free memory in shared mapping");
    }
    ...
    fn enter(&mut self, _gdblisten: &Option<String>) -> Result<Command> {
        ...
        match vcpu_fd.run()? {
            ...
            VcpuExit::Hypercall(HypercallExit {
                ...
                let res = execute_page_state_change(
                    address,
                    num_pages,
                    &mut *self.keep.write().unwrap(),
                    private,
                );
                match res {
                    Ok(_) => *ret = 0,
